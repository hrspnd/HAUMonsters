...
// Auth check — only players allowed
...
// Player identity from session
$player_id   = $_SESSION['player_id'];
$player_name = $_SESSION['username'];

// ── STAT 1: My Monsters (total caught by this player) ──────────────────────
$stmt        = $conn->prepare("SELECT COUNT(*) AS total FROM monster_catchestbl WHERE player_id = ?");
$stmt->bind_param("i", $player_id);
$stmt->execute();
$result      = $stmt->get_result();
$my_monsters = $result ? $result->fetch_assoc()['total'] : 0;

// ── STAT 2: My Ranking (rank by total monsters caught) ─────────────────────
$stmt = $conn->prepare("
    ...
    WHERE total > (
        SELECT COUNT(*) FROM monster_catchestbl WHERE player_id = ?
    )
");
$stmt->bind_param("i", $player_id);
$stmt->execute();
$result  = $stmt->get_result();
$my_rank = $result ? '#' . $result->fetch_assoc()['rank'] : '#—';
...
<title>Player Dashboard — HAUPokémon</title>
...
.stat-ribbon {
  ...
  border-width: 0 52px 52px 0; opacity: 0.2;
}
...
.bottom-bar-right a {
  ...
  color: var(--blue-light); letter-spacing: .08em; opacity: .7;
  text-decoration: none;
  transition: color 0.18s, opacity 0.18s;
}
.bottom-bar-right a:hover { color: var(--yellow); opacity: 1; }
...
<main class="main">
  ...
  <div class="page-header">
    <div>
      <div class="page-eyebrow">// TRAINER HUB</div>
      <div class="page-title-main">Welcome, <span><?php echo htmlspecialchars($player_name); ?></span>.</div>
    </div>
  </div>
  ...
  <div class="stat-strip">
    ...
    <div class="stat-card">
      ...
      <div class="stat-top-row">
        ...
        <div class="stat-type-chip">CAUGHT</div>
      </div>
      <div class="stat-value"><?php echo htmlspecialchars($my_monsters); ?></div>
      <div class="stat-label">My Monsters</div>
      <div class="stat-sub">Total caught</div>
    ...
    <div class="stat-card">
      ...
      <div class="stat-top-row">
        <div class="stat-icon-pill">🏅</div>
        <div class="stat-type-chip">RANK</div>
      </div>
      <div class="stat-value"><?php echo htmlspecialchars($my_rank); ?></div>
      <div class="stat-label">My Ranking</div>
      <div class="stat-sub">Global leaderboard</div>
      <div class="stat-bar-wrap">
        <span class="stat-bar-label">HP</span>
        <div class="stat-bar-track"><div class="stat-bar-fill" style="width:72%"></div></div>
      </div>
    </div>
  ...
  <div class="nav-grid">
    ...
    <a class="nav-card" href="catch_monsters.php" onclick="nav('Catch Monsters')">
      ...
      <div class="nav-content">
        <div class="nav-title">Catch Monsters</div>
        <div class="nav-desc">Find & catch monsters</div>
        <div class="nav-type-badge">GOTTA CATCH 'EM ALL!</div>
      </div>
      <div class="nav-arrow">▶</div>
    </a>

    <a class="nav-card" href="player_monsters.php" onclick="nav('My Monsters')">
      <div class="nav-stripe"></div>
      <svg class="nav-pb" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
        <circle cx="50" cy="50" r="48" fill="none" stroke="#cc0000" stroke-width="6"/>
        <path d="M2,50 A48,48 0 0,1 98,50" fill="#cc0000"/>
      ...
      <div class="nav-icon-wrap">🧢</div>
      <div class="nav-content">
        <div class="nav-title">My Monsters</div>
        <div class="nav-desc">View your caught monsters</div>
        <div class="nav-type-badge">POKEDÉX</div>
      </div>
      <div class="nav-arrow">▶</div>
    </a>
  ...
  <div style="animation:fadeUp .5s .44s ease both;">
    <a class="nav-card danger" href="logout.php" onclick="nav('Logging out...')">
      ...
      <div class="nav-content">
        ...
        <div class="nav-desc">End player session</div>
